...
const logDirectory = path.join(os.homedir(), '.boggle', 'logs');
...
const logger = createLogger({
  ...
  transports: [
    new transports.DailyRotateFile({
      filename: path.join(logDirectory, 'boggle-app-%DATE%.log'),
  ...
  exceptionHandlers: [
    new transports.DailyRotateFile({
      filename: path.join(logDirectory, 'boggle-exceptions-%DATE%.log'),
      datePattern: 'YYYY-MM-DD',
      maxSize: '10m',
      maxFiles: '7d'
    })
